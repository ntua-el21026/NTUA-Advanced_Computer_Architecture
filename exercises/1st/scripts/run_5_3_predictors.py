#!/usr/bin/env python3
"""Run and summarize Assignment 1 section 5.3 branch predictors.

The script runs cslab_branch.so with -predictor_set 5.3 on the train inputs
and stores:
  - raw pintool output in benchmarks/5.3/train/raw/<benchmark>.txt
  - application stdout/stderr logs in benchmarks/5.3/train/logs/
  - per-benchmark predictor metrics in benchmarks/5.3/summary.csv
  - per-predictor averages in benchmarks/5.3/summary_by_predictor.csv
  - per-question grouped summaries in benchmarks/5.3/summary_by_group.csv
  - a readable text summary in benchmarks/5.3/summary.txt
...
PREDICTOR_GROUPS = {
    "5.3.i-fixed-16K-entries": [
        "Nbit-16K-1",
        "Nbit-16K-2",
        "Nbit-16K-3",
        "Nbit-16K-4",
    ],
    "5.3.ii-2bit-nair-fsms": [
        "Nbit-16K-2",
        "FSM-16K-BCBAADCD-3",
        "FSM-16K-BCBABDCD-3",
        "FSM-16K-BCBDACBA-10",
...
EXPECTED_PREDICTORS = sorted({name for names in PREDICTOR_GROUPS.values() for name in names})
...
def run_one(
    ...
    pin_cmd = (
        ...
        f"-o {shell_quote(run.raw_output.resolve())} -predictor_set 5.3 -- "
        f"{run.command} "
...
def predictor_groups(predictor: str) -> list[str]:
    return [group for group, names in PREDICTOR_GROUPS.items() if predictor in names]
...
def collect_rows(runs: list[BenchRun]) -> tuple[list[dict[str, str]], list[BenchRun]]:
    ...
    for run in runs:
        ...
        for predictor in EXPECTED_PREDICTORS:
            ...
            groups = predictor_groups(predictor)
            rows.append(
                {
...
def aggregate_rows(rows: list[dict[str, str]], group: str | None = None) -> list[dict[str, str]]:
    predictors = EXPECTED_PREDICTORS
    if group is not None:
        predictors = PREDICTOR_GROUPS[group]

    output: list[dict[str, str]] = []
    for predictor in predictors:
        selected = [row for row in rows if row["predictor"] == predictor and (group is None or group in row["groups"].split(";"))]
        if not selected:
            continue
        ...
        if group is None:
            common["groups"] = ";".join(predictor_groups(predictor))
        else:
            common["group"] = group
...
def write_summaries(runs: list[BenchRun], output_root: Path) -> None:
    ...
    by_group_path = output_root / "summary_by_group.csv"
    ...
    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Assignment 1 - Section 5.3 N-bit Predictors\n")
        ...
        for group in PREDICTOR_GROUPS:
            ...
            handle.write(f"{'Predictor':<26} {'Mean MPKI':>12} {'Agg MPKI':>12} {'Mean Acc%':>12}\n")
            handle.write("-" * 66 + "\n")
            for row in aggregate_rows(rows, group=group):
                handle.write(
                    f"{row['predictor']:<26} "
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=first_dir / "benchmarks" / "5.3")
