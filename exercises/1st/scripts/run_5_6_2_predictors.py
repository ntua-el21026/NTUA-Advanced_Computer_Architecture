#!/usr/bin/env python3
"""Run and summarize Assignment 1 section 5.6.2 predictor comparison.

The script runs cslab_branch.so with -predictor_set 5.6.2 on the train inputs
and stores:
  - raw pintool output in benchmarks/5.6.2/train/raw/<benchmark>.txt
  - application stdout/stderr logs in benchmarks/5.6.2/train/logs/
  - per-benchmark predictor metrics in benchmarks/5.6.2/summary.csv
  - per-predictor averages in benchmarks/5.6.2/summary_by_predictor.csv
  - per-family averages in benchmarks/5.6.2/summary_by_family.csv
  - a readable text summary in benchmarks/5.6.2/summary.txt
...
PREDICTORS = [
    {"name": "Static-AlwaysTaken", "family": "static", "hardware_bits": 0},
    {"name": "Static-BTFNT", "family": "static", "hardware_bits": 0},
    {"name": "Nbit-16K-2", "family": "nbit", "hardware_bits": 32768},
    {"name": "Pentium-M", "family": "pentium-m", "hardware_bits": 30000},
    {"name": "Local-X2048-Z8-PHT8K-2", "family": "local-history", "hardware_bits": 32768},
    {"name": "Local-X4096-Z4-PHT8K-2", "family": "local-history", "hardware_bits": 32768},
    {"name": "Local-X8192-Z2-PHT8K-2", "family": "local-history", "hardware_bits": 32768},
    {"name": "Global-PHT16K-BHR4-2", "family": "global-history", "hardware_bits": 32768},
    {"name": "Global-PHT16K-BHR8-2", "family": "global-history", "hardware_bits": 32768},
    {"name": "Global-PHT16K-BHR12-2", "family": "global-history", "hardware_bits": 32768},
    {"name": "Perceptron-M728-N8", "family": "perceptron", "hardware_bits": 32760},
    ...
    {"name": "Tournament-M1024-Nbit16K1-Global8K-BHR8", "family": "tournament", "hardware_bits": 32768},
    {"name": "Tournament-M1024-Local2048x4-Global8K-BHR8", "family": "tournament", "hardware_bits": 32768},
    {"name": "Tournament-M2048-Nbit8K2-Perceptron16K-N8", "family": "tournament", "hardware_bits": 32768},
    {"name": "Tournament-M2048-Local1024x6-Perceptron16K-N8", "family": "tournament", "hardware_bits": 30716},
]
...
def run_one(
    ...
    pin_cmd = (
        ...
        f"-o {shell_quote(run.raw_output.resolve())} -predictor_set 5.6.2 -- "
        f"{run.command} "
...
def aggregate_by_family(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    output: list[dict[str, str]] = []
    families = sorted({str(item["family"]) for item in PREDICTORS})
    for family in families:
        ...
        output.append(
            {
                "family": family,
                "predictors": str(len({row["predictor"] for row in selected})),
...
def write_summaries(runs: list[BenchRun], output_root: Path) -> None:
    ...
    by_family_path = output_root / "summary_by_family.csv"
    ...
    with by_family_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=BY_FAMILY_FIELDS)
        writer.writeheader()
        writer.writerows(by_family)

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Assignment 1 - Section 5.6.2 Predictor Comparison\n")
        ...
        handle.write("Predictors\n")
        handle.write(f"{'Predictor':<50} {'Family':<16} {'Bits':>8} {'Mean MPKI':>12} {'Agg MPKI':>12} {'Mean Acc%':>12}\n")
        handle.write("-" * 115 + "\n")
        ...
        handle.write("\nFamilies\n")
        handle.write(f"{'Family':<16} {'Predictors':>10} {'Rows':>8} {'Mean MPKI':>12} {'Agg MPKI':>12} {'Mean Acc%':>12}\n")
        handle.write("-" * 78 + "\n")
        for row in by_family:
            handle.write(
                f"{row['family']:<16} "
                f"{int(row['predictors']):>10} "
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=first_dir / "benchmarks" / "5.6.2")
