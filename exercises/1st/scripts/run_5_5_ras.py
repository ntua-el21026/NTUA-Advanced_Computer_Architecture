#!/usr/bin/env python3
"""Run and summarize Assignment 1 section 5.5 RAS configurations.

The script runs cslab_branch.so with -predictor_set 5.5 on the train inputs
and stores:
  - raw pintool output in benchmarks/5.5/train/raw/<benchmark>.txt
  - application stdout/stderr logs in benchmarks/5.5/train/logs/
  - per-benchmark RAS metrics in benchmarks/5.5/summary.csv
  - per-RAS-size averages in benchmarks/5.5/summary_by_ras.csv
  - a readable text summary in benchmarks/5.5/summary.txt
...
RAS_RE = re.compile(r"^\s*RAS \((\d+) entries\):\s+(\d+)\s+(\d+)\s*$")
...
BY_RAS_FIELDS = [
    "ras_entries",
    "benchmarks",
    "arithmetic_mean_miss_rate_pct",
    "aggregate_miss_rate_pct",
    "arithmetic_mean_accuracy_pct",
    "arithmetic_mean_ras_miss_mpki",
    "aggregate_ras_miss_mpki",
    "total_instructions",
    "total_returns",
    "total_incorrect",
]
...
def parse_raw_output(path: Path) -> tuple[int, dict[int, tuple[int, int]]] | None:
    if not path.exists() or path.stat().st_size == 0:
        return None

    total_instructions: int | None = None
    ras_stats: dict[int, tuple[int, int]] = {}
    in_ras_section = False
    ...
    with path.open("r", encoding="utf-8", errors="replace") as handle:
        for line in handle:
            ...
            if in_ras_section:
                ...
                ras_match = RAS_RE.match(line)
                if ras_match:
                    ras_stats[int(ras_match.group(1))] = (int(ras_match.group(2)), int(ras_match.group(3)))

    if total_instructions is None:
        return None
    if not set(RAS_SIZES).issubset(ras_stats):
        return None

    return total_instructions, ras_stats
...
def run_one(
    ...
    pin_cmd = (
        ...
        f"-o {shell_quote(run.raw_output.resolve())} -predictor_set 5.5 -- "
        f"{run.command} "
...
def collect_rows(runs: list[BenchRun]) -> tuple[list[dict[str, str]], list[BenchRun]]:
    ...
    for run in runs:
        ...
        total_instructions, ras_stats = parsed
        for ras_entries in RAS_SIZES:
            correct, incorrect = ras_stats[ras_entries]
            returns = correct + incorrect
            rows.append(
                {
                    "benchmark": run.benchmark,
                    "ras_entries": str(ras_entries),
                    "correct": str(correct),
                    "incorrect": str(incorrect),
                    "returns": str(returns),
                    "total_instructions": str(total_instructions),
                    "miss_rate_pct": f"{pct(incorrect, returns):.6f}",
                    "accuracy_pct": f"{pct(correct, returns):.6f}",
                    "ras_miss_mpki": f"{mpki(incorrect, total_instructions):.6f}",
...
def aggregate_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for ras_entries in RAS_SIZES:
        selected = [row for row in rows if int(row["ras_entries"]) == ras_entries]
        if not selected:
            continue

        total_instructions = sum(int(row["total_instructions"]) for row in selected)
        total_returns = sum(int(row["returns"]) for row in selected)
        total_incorrect = sum(int(row["incorrect"]) for row in selected)
        output.append(
            {
                "ras_entries": str(ras_entries),
                "benchmarks": str(len(selected)),
                "arithmetic_mean_miss_rate_pct": f"{sum(float(row['miss_rate_pct']) for row in selected) / len(selected):.6f}",
                "aggregate_miss_rate_pct": f"{pct(total_incorrect, total_returns):.6f}",
                "arithmetic_mean_accuracy_pct": f"{sum(float(row['accuracy_pct']) for row in selected) / len(selected):.6f}",
                "arithmetic_mean_ras_miss_mpki": f"{sum(float(row['ras_miss_mpki']) for row in selected) / len(selected):.6f}",
                "aggregate_ras_miss_mpki": f"{mpki(total_incorrect, total_instructions):.6f}",
                "total_instructions": str(total_instructions),
...
def write_summaries(runs: list[BenchRun], output_root: Path) -> None:
    ...
    by_ras_path = output_root / "summary_by_ras.csv"
    ...
    by_ras = aggregate_rows(rows)
    with by_ras_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=BY_RAS_FIELDS)
        writer.writeheader()
        writer.writerows(by_ras)

    with txt_path.open("w", encoding="utf-8") as handle:
        handle.write("Assignment 1 - Section 5.5 RAS Study\n")
        ...
        handle.write(f"{'RAS entries':>11} {'Mean miss%':>12} {'Agg miss%':>12} {'Mean MPKI':>12} {'Agg MPKI':>12}\n")
        handle.write("-" * 65 + "\n")
        for row in by_ras:
            handle.write(
                f"{int(row['ras_entries']):>11} "
                f"{float(row['arithmetic_mean_miss_rate_pct']):>12.6f} "
                f"{float(row['aggregate_miss_rate_pct']):>12.6f} "
                f"{float(row['arithmetic_mean_ras_miss_mpki']):>12.6f} "
                f"{float(row['aggregate_ras_miss_mpki']):>12.6f}\n"
            )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path, default=first_dir / "benchmarks" / "5.5")
